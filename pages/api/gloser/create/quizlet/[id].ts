...
async function handler(req: NextApiRequest, res) {
  ...
  for (let itemIndex in quizletRes.data.responses[0].models.studiableItem) {
    ...
    pinyin.split("").forEach((char) => {
      if (isHanzi(char)) {
        return res.status(500).json({ message: "Unable to exctract hanzi, pinyin includes hanzi" });
      }
    });

    // Check if hanzi is a valid hanzi
    if (!isHanzi(hanzi)) {
      return res.status(500).json({ message: "Unable to exctract hanzi. Error occured on hanzi: " + hanzi });
    }
